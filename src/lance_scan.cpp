...
LanceScanLocalInit(ExecutionContext &context, TableFunctionInitInput &input,
  ...
  return make_uniq_base<LocalTableFunctionState, ArrowScanLocalState>(
      std::move(chunk), context.client);
}
...
static void LanceScanFunc(ClientContext &context, TableFunctionInput &data,
  ...
  if (scan_state.chunk_offset >=
      NumericCast<idx_t>(scan_state.chunk->arrow_array.length)) {
  ...
  auto remaining = NumericCast<idx_t>(scan_state.chunk->arrow_array.length) -
                   scan_state.chunk_offset;
  auto output_size = MinValue<idx_t>(STANDARD_VECTOR_SIZE, remaining);
  output.SetCardinality(output_size);

  ArrowTableFunction::ArrowToDuckDB(scan_state,
                                    bind_data.arrow_table.GetColumns(), output,
                                    global_state.lines_read);
